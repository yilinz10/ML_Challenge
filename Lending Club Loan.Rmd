...
loan_data %>%  
  ggplot(aes(x = loan_amount, y = ..density..)) +
...
loan_data[sample(nrow(loan_data) , 100000) , ] %>%
...
trn_label = as.numeric(loan_trn_down$target) - 1
...
X_trn = xgb.DMatrix(data = new_trn, label = trn_label)
X_tst = xgb.DMatrix(data = new_tst, label = tst_label)
...
pred_trn = factor(ifelse(predict(xgb_mod, X_trn) > 0.5, "bad", "good"), levels = c("good", "bad"))
confusionMatrix(data = pred_trn,
                reference = loan_trn_down$target,
                positive = "bad")
pred_tst = factor(ifelse(predict(xgb_mod, X_tst) > 0.5, "bad", "good"), levels = c("good", "no"))
confusionMatrix(data = pred_tst,
