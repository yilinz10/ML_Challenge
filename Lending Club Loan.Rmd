...
set.seed(316)
loan_trn_down = downSample(x = loan_trn[, -55], 
                           y = loan_trn$target,
                           yname = "target")

new_trn = model.matrix(~.+0, data = loan_trn_down[, -55])
new_tst = model.matrix(~.+0, data = loan_tst[, -55])
...
confusionMatrix(data = predict(xgb_mod, X_trn)
